defmodule Frobots.AccountsTest do
  ...
  describe "users" do
    ...
    # # change from usernames to email
    test "does not accept long emails" do
      attrs = Map.put(@valid_attrs, :email, String.duplicate("a", 321) <> "@mail.com")
      {:error, changeset} = Accounts.register_user(attrs)

      # change from 320 to 160. Schema tests for email lenth max 160
    ...
    test "create_user/1 with valid data creates a user" do
      valid_attrs = %{name: "some name", email: "test@email.com", password: "secret123"}
    ...
    test "update_user_password/3 with valid data updates the user password" do
      {:ok, user} = user_fixture()
      # name: "some new name",
    ...
    test "update_user/2 with valid data updates the user" do
      ...
      assert {:ok, %User{} = user} = Accounts.update_profile(user, update_attrs)
      assert user.name == "some new name"
      assert user.email == "somenew@email.com"
    end
    ...
    test "update_user/2 with invalid data returns error changeset" do
      ...
      assert {:error, %Ecto.Changeset{}} =
               Accounts.update_user_password(user, dummy_password(), %{password: "pass"})

      assert user == Accounts.get_user!(user.id)
    end

    # TODO add delete method to Accounts
  ...
  describe "authenticate_by_username_and_pass/2" do
    ...
    test "returns nil with invalid password", %{user: user} do
      # assert {:error, :unauthorized} =
      assert is_nil(Accounts.get_user_by_email_and_password(user.email, "badpass"))
    end

    #   # test "returns not found error with no matching user for email" do
    test "returns nil with no matching user for email" do
      assert is_nil(Accounts.get_user_by_email_and_password("unknownuser", @pass))
    end
  end

  import Frobots.AccountsFixtures
  alias Frobots.Accounts.{User, UserToken}
  ...
  describe "get_user_by_email_and_password/2" do
    ...
    test "does not return the user if the password is not valid" do
      {:ok, user} = user_fixture()
      refute Accounts.get_user_by_email_and_password(user.email, "invalidpass")
      # assert is_nil(Accounts.get_user_by_email_and_password(user.email, "invalid11"))
    end

    test "returns the user if the email and password are valid" do
      {:ok, user} = user_fixture()
      db_user = Accounts.get_user_by_email_and_password(user.email, dummy_password())
  ...
  describe "apply_user_email/3" do
    ...
    test "validates email uniqueness", %{user: user} do
      ...
      {:error, changeset} =
        Accounts.apply_user_email(user, valid_user_password(), %{email: new_user.email})
    ...
    test "applies the email without persisting it", %{user: user} do
      email = unique_user_email()
      {:ok, user} = Accounts.apply_user_email(user, dummy_password(), %{email: email})
      assert user.email == email
  ...
  describe "update_user_password/3" do
    ...
    test "updates the password", %{user: user} do
      {:ok, user} =
        Accounts.update_user_password(user, "supersecret", %{
          password: "new valid password"
        })

      # assert is_nil(user.password) ..why?
    ...
    test "deletes all tokens for the given user", %{user: user} do
      ...
      {:ok, _} =
        Accounts.update_user_password(user, dummy_password(), %{
          password: "new valid password"
        })

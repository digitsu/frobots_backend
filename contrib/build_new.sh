...
docker run --rm -dp $PORT:$PORT -e LINODE_BACKEND_STORE_URL -e LINODE_BACKEND_STORE_ACCESS_KEY -e LINODE_BACKEND_STORE_SECRET_KEY -e GHOST_API_KEY -e SENDGRID_API_KEY -e SENDGRID_API_EXPORT_MAILINGLIST_KEY -e POOL_SIZE -e PORT -e DATABASE_URL=$DATABASE_URL_NEW${dbname} -e SECRET_KEY_BASE -e ADMIN_USER -e ADMIN_PASS --network $FROBOTS_NETWORK --network-alias frobots_backend -v web_certs:/var/certs --name frobots_backend elixir/frobots_backend
